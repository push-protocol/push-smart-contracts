...
contract DaoHarvest_test is BaseFuzzStaking {
    ...
    function test_AdminHarvest(uint256 _fee, uint256 _passEpoch) public {
        ...
        uint256 rewards = feePoolStaking.usersRewardsClaimed(address(coreProxy));
    ...
    function test_AdminHarvestZeroReward(uint256 _passEpoch) public {
        ...
        daoHarvest(actor.admin, _passEpoch - 1);
        uint256 rewardsBef = feePoolStaking.usersRewardsClaimed(address(coreProxy));
    ...
    function test_NoStakerDaoGetsRewards(uint256 _passEpoch, uint256 _fee) public {
        ...
        uint256 claimed = feePoolStaking.usersRewardsClaimed(address(coreProxy));
