...
contract Migration_Test is BasePushCoreTest {
    ...
    function test_migration() external {
        ...
        for (uint i; i < _channels.length; ++i) {
            (
                ,
            ...
            console.log(
                ...
                actualChannelStartBlock,
                actualChannelUpdateBlock
            );
        }
        changePrank(actor.admin);
        epnsCoreProxyAdmin.upgrade(
            ITransparentUpgradeableProxy(address(epnsCoreProxyV2)),
            address(coreV3)
        );
        ...
        vm.expectRevert();
        coreV3.migaretAddresToBytes32(_channels);

        coreV3.pauseContract();
        coreV3.migaretAddresToBytes32(_channels);
        for (uint i; i < _channelsBytes.length; ++i) {
            (
                ,
                uint8 actualChannelState,
                ,
