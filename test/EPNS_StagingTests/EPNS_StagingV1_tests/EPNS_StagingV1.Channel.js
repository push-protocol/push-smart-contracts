...
await EPNSCoreV1Proxy.connect(ADMINSIGNER).addToChannelizationWhitelist(CHANNEL_CREATOR, {gasLimit: 500000});
          const tx = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFees(CHANNEL_TYPE_SECOND, testChannelSecond,{gasLimit: 500000});
		...
		await EPNSCoreV1Proxy.connect(ADMINSIGNER).addToChannelizationWhitelist(CHANNEL_CREATOR, {gasLimit: 500000});
          const tx1 = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFees(CHANNEL_TYPE, testChannel, {gasLimit: 500000});
          ...
          const testChannelSecond = ethers.utils.toUtf8Bytes("test-channel-hello-world-two");
			await EPNSCoreV1Proxy.connect(ADMINSIGNER).addToChannelizationWhitelist(CHANNEL_CREATOR, {gasLimit: 500000});
          const tx2 = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFees(CHANNEL_TYPE_SECOND, testChannelSecond,{gasLimit: 500000});
      ...
      // UPDATING CHANNEL DETAILS ON CHAIN
        ...
        await EPNSCoreV1Proxy.connect(ADMINSIGNER).addToChannelizationWhitelist(CHANNEL_CREATOR, {gasLimit: 500000});
        const tx = await EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFees(CHANNEL_TYPE, testChannel,{gasLimit: 500000});
      ...
      it("Should emit AddChannel event when creating channel", async function(){
        ...
        const tx = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFees(CHANNEL_TYPE, testChannel,{gasLimit: 500000});
   ...
   describe("Testing createChannelWithFeesAndPublicKey", function(){
      ...
      it("should revert on channel creation when User already a channel owner", async function () {
        ...
        await EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE, testChannel, publicKey.slice(1), {gasLimit: 500000});

        const CHANNEL_TYPE_SECOND = 3;
        const testChannelSecond = ethers.utils.toUtf8Bytes("test-channel-hello-world-two");

        const tx = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE_SECOND, testChannelSecond, publicKey.slice(1), {gasLimit: 500000});
      ...
      it("should revert on channel creation when user not allowed channel type", async function () {
        ...
        const publicKey = await getPubKey(CHANNEL_CREATORSIGNER)
        const tx1 = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE, testChannel, publicKey.slice(1), {gasLimit: 500000});
        ...
        const tx2 = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE_SECOND, testChannelSecond, publicKey.slice(1), {gasLimit: 500000});
      ...
      it("Should broadcast user public key when creating channel", async function(){
        ...
        const publicKey = await getPubKey(CHANNEL_CREATORSIGNER)
        const tx = await EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE, testChannel, publicKey.slice(1), {gasLimit: 500000});
      ...
      it("should emit PublicKeyRegistered event when user public key is not registered", async function(){
        ...
        const publicKey = await getPubKey(CHANNEL_CREATORSIGNER)
        const tx = EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithFeesAndPublicKey(CHANNEL_TYPE, testChannel, publicKey.slice(1), {gasLimit: 500000});
