...
describe("AdjustChannelPoolContributions Test", function () {
  ...
  beforeEach(async function () {
    const [adminSigner, aliceSigner, bobSigner, charlieSigner, abySigner] =
    ...
    // Trasnfers
    await MOCKDAI.connect(DAI_WHALE_SIGNER).transfer(
      ALICE,
      ADD_CHANNEL_MIN_FEES.mul(100)
    );
    await MOCKDAI.connect(DAI_WHALE_SIGNER).transfer(
      BOB,
      ADD_CHANNEL_MIN_FEES.mul(100)
    ...
    // Approve
    await MOCKDAI.connect(ALICESIGNER).approve(
      EPNSCoreV1Proxy.address,
      ADD_CHANNEL_MIN_FEES
    );
    await MOCKDAI.connect(BOBSIGNER).approve(
      EPNSCoreV1Proxy.address,
      ADD_CHANNEL_MIN_FEES
    );
    await MOCKDAI.connect(CHARLIESIGNER).approve(
      EPNSCoreV1Proxy.address,
      ADD_CHANNEL_MIN_FEES.mul(10)
    );
    await MOCKDAI.connect(ABYSIGNER).approve(
      EPNSCoreV1Proxy.address,
      ADD_CHANNEL_MIN_FEES.mul(10)
    );
    ...
    TempStoreContract = await ethers
      .getContractFactory("TempStorage")
  ...
  it("Updates channel poolContribution adequately", async () => {
    ...
    const aliceExpectedContribution = ADD_CHANNEL_MIN_FEES.mul(newPoolFunds)
      .div(oldPoolFunds)
      .sub(FEE_AMOUNT);
    const bobExpectedContribution = ADD_CHANNEL_MIN_FEES.mul(newPoolFunds)
      .div(oldPoolFunds)
      .sub(FEE_AMOUNT);
    const charlieExpectedContribution = ADD_CHANNEL_MIN_FEES.mul(10)
      .mul(newPoolFunds)
      .div(oldPoolFunds)
      .sub(FEE_AMOUNT);
    const abyExpectedContribution = ADD_CHANNEL_MIN_FEES.mul(6)
      .mul(newPoolFunds)
      .div(oldPoolFunds)
      .sub(FEE_AMOUNT);

    const aliceChannelNewPoolContrib = await EPNSCoreV1Proxy.channels(
      ALICE
    ).then((d) => d.poolContribution);
    const bobChannelNewPoolContrib = await EPNSCoreV1Proxy.channels(BOB).then(
      (d) => d.poolContribution
    );
    const charlieChannelNewPoolContrib = await EPNSCoreV1Proxy.channels(
      CHARLIE
    ).then((d) => d.poolContribution);
    const abyChannelNewPoolContrib = await EPNSCoreV1Proxy.channels(ABY).then(
      (d) => d.poolContribution
    );

    const aliceChannelNewWeight = await EPNSCoreV1Proxy.channels(ALICE).then(
      (d) => d.channelWeight
    );
    expect(aliceExpectedContribution).to.be.closeTo(
      aliceChannelNewPoolContrib,
      utils.parseEther("0.0001")
    );
    expect(bobExpectedContribution).to.be.closeTo(
      bobChannelNewPoolContrib,
      utils.parseEther("0.0001")
    );
    expect(charlieExpectedContribution).to.be.closeTo(
      charlieChannelNewPoolContrib,
      utils.parseEther("0.0001")
    );
    expect(abyExpectedContribution).to.be.closeTo(
      abyChannelNewPoolContrib,
      utils.parseEther("0.0001")
    );
  });
  ...
  it("Updates channel poolWeight properly", async () => {
    ...
    const aliceExpectedWeight = aliceExpectedContribution
      .mul(ADJUST_FOR_FLOAT)
      .div(MIN_POOL_CONTRIBUTION);
    const bobExpectedWeight = bobExpectedContribution
      .mul(ADJUST_FOR_FLOAT)
      .div(MIN_POOL_CONTRIBUTION);
    const charlieExpectedWeight = charlieExpectedContribution
      .mul(ADJUST_FOR_FLOAT)
      .div(MIN_POOL_CONTRIBUTION);
    const abyExpectedWeight = abyExpectedContribution
      .mul(ADJUST_FOR_FLOAT)
      .div(MIN_POOL_CONTRIBUTION);
    ...
    const bobChannelNewWeight = await EPNSCoreV1Proxy.channels(BOB).then(
      (d) => d.channelWeight
    );
    const charlieChannelWeight = await EPNSCoreV1Proxy.channels(CHARLIE).then(
      (d) => d.channelWeight
    );
    const abyChannelWeight = await EPNSCoreV1Proxy.channels(ABY).then(
      (d) => d.channelWeight
    );
  ...
  it("Assignes correct channelUpdateBlock value ", async () => {
    ...
    const aliceChannelUpdateBlock = await EPNSCoreV1Proxy.channels(ALICE).then(
      (d) => d.channelUpdateBlock
    );
    const bobChannelUpdateBlock = await EPNSCoreV1Proxy.channels(BOB).then(
      (d) => d.channelUpdateBlock
    );
    const charlieChannelUpdateBlock = await EPNSCoreV1Proxy.channels(
      CHARLIE
    ).then((d) => d.channelUpdateBlock);
    const abyChannelUpdateBlock = await EPNSCoreV1Proxy.channels(ABY).then(
      (d) => d.channelUpdateBlock
    );
  ...
  it("Should not update non exsting channel", async () => {
    ...
    const falseChannelContribInitial = await EPNSCoreV1Proxy.channels(
      FALSE_CHANNEL_ADDRESS
    ).then((d) => d.poolContribution);
    ...
    const falseChannelContribFinal = await EPNSCoreV1Proxy.channels(
      FALSE_CHANNEL_ADDRESS
    ).then((d) => d.poolContribution);
  ...
  it("Shall not execute if Caller is not the PUSH Channel Admin", async () => {
    ...
    const newPoolFunds_before = await EPNSCoreV1Proxy.CHANNEL_POOL_FUNDS();
    // without pause txn will fail
    const txn1 = EPNSCoreV1Proxy.connect(
      BOBSIGNER
    ).adjustChannelPoolContributions(
      TempStoreContract.address, //Temp Contract
      0, // start index
      2, // end index
      oldPoolFunds,
    ...
    await expect(txn1).to.be.revertedWith(
      "EPNSCoreV1::onlyPushChannelAdmin: Caller not pushChannelAdmin"
    );

    // admin updates channel pool contribution
    const tx = await EPNSCoreV1Proxy.connect(
      ADMINSIGNER
    ).adjustChannelPoolContributions(
      TempStoreContract.address, //Temp Contract
      0, // start index
