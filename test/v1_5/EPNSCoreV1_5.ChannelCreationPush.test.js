  ...
  describe("EPNS CORE: Channel Creation Tests", function () {
    describe("Testing the Base Create Channel Function", function () {
      ...
      it("Should revert if IF EPNSCoreV1::onlyInactiveChannels: Channel already Activated ", async function () {
        ...
        const tx = EPNSCoreV1Proxy.connect(
          CHANNEL_CREATORSIGNER
        ).createChannelWithPUSH(
          CHANNEL_TYPE,
          testChannel,
      ...
      it("Should revert Channel Type is not the ALLOWED TYPES", async function () {
        ...
        const tx1 = EPNSCoreV1Proxy.connect(
          CHANNEL_CREATORSIGNER
        ).createChannelWithPUSH(
          CHANNEL_TYPE,
          testChannel,
        ...
        const testChannelSecond = ethers.utils.toUtf8Bytes(
          "test-channel-hello-world-two"
        );

        const tx2 = EPNSCoreV1Proxy.connect(
          CHANNEL_CREATORSIGNER
        ).createChannelWithPUSH(
          CHANNEL_TYPE_SECOND,
          testChannelSecond,
      ...
      it("should transfer given fees from creator account to proxy", async function () {
        ...
        const pushBalanceBefore_user = await PushToken.balanceOf(
          CHANNEL_CREATOR
        );
        const pushBalanceBefore_coreContract = await PushToken.balanceOf(
          EPNSCoreV1Proxy.address
        );
        ...
        const pushBalanceAfter_user = await PushToken.balanceOf(
          CHANNEL_CREATOR
        );
        const pushBalanceAfter_coreContract = await PushToken.balanceOf(
          EPNSCoreV1Proxy.address
        );

        expect(pushBalanceBefore_user.sub(pushBalanceAfter_user)).to.equal(
          pushBalanceAfter_coreContract.sub(pushBalanceBefore_coreContract)
        );
      });
      ...
      it("CHANNEL_POOL_FUNDS & PROTOCOL_POOL_FEES should be updated correctly for Multiple Channel Creation", async function () {
        ...
        await EPNSCoreV1Proxy.connect(BOBSIGNER).createChannelWithPUSH(
          CHANNEL_TYPE,
          testChannel,
          tokensBN(150),
          0
        );
        await EPNSCoreV1Proxy.connect(ALICESIGNER).createChannelWithPUSH(
          CHANNEL_TYPE,
          testChannel,
          tokensBN(290),
          0
        );
        await EPNSCoreV1Proxy.connect(CHARLIESIGNER).createChannelWithPUSH(
          CHANNEL_TYPE,
          testChannel,
          tokensBN(500),
          0
        ...
        const totalAmounDeposited = ADD_CHANNEL_MIN_FEES.add(
          tokensBN(150).add(tokensBN(290).add(tokensBN(500)))
        );
      ...
      it("EPNS Core Should Interact with EPNS Communcator and make the necessary Subscriptions", async function () {
        ...
        const isChannelOwnerSubscribed_before =
          await EPNSCommV1Proxy.isUserSubscribed(
            CHANNEL_CREATOR,
            CHANNEL_CREATOR
          );
        const isChannelSubscribedToEPNS_before =
          await EPNSCommV1Proxy.isUserSubscribed(EPNS_ALERTER, CHANNEL_CREATOR);
        const isAdminSubscribedToChannel_before =
          await EPNSCommV1Proxy.isUserSubscribed(CHANNEL_CREATOR, ADMIN);
        ...
        const isChannelOwnerSubscribed_after =
          await EPNSCommV1Proxy.isUserSubscribed(
            CHANNEL_CREATOR,
            CHANNEL_CREATOR
          );
        const isChannelSubscribedToEPNS_after =
          await EPNSCommV1Proxy.isUserSubscribed(EPNS_ALERTER, CHANNEL_CREATOR);
        const isAdminSubscribedToChannel_after =
          await EPNSCommV1Proxy.isUserSubscribed(CHANNEL_CREATOR, ADMIN);
      ...
      it("Function Should emit Relevant Events", async function () {
        ...
        await expect(tx)
          ...
          .withArgs(
            CHANNEL_CREATOR,
            CHANNEL_TYPE,
            ethers.utils.hexlify(testChannel)
          );
      });
    });
  });
