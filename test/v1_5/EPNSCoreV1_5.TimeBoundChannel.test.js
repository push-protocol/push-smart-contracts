  ...
  describe("EPNS CORE: Channel Creation Tests", function () {
    describe("Testing the Base Create Channel Function", function () {
      ...
      const passTime = async (time) => {
        const numBlock = Math.round(time / BLOCK_TIME) + 1;
        await passBlockNumers(numBlock);
      ...
      it("Should revert on creating channel with invalid expiry", async function () {
        await expect(
          EPNSCoreV1Proxy.connect(CHANNEL_CREATORSIGNER).createChannelWithPUSH(
            TIME_BOUND_CHANNEL_TYPE,
            testChannel,
            ADD_CHANNEL_MIN_POOL_CONTRIBUTION,
            0
      ...
      it("It allows creator to destroy the time bound channel", async function () {
        ...
        const txn = EPNSCoreV1Proxy.connect(
          CHANNEL_CREATORSIGNER
        ).destroyTimeBoundChannel(CHANNEL_CREATOR);

        await expect(txn)
          .to.emit(EPNSCoreV1Proxy, "TimeBoundChannelDestroyed")
          .withArgs(
            CHANNEL_CREATOR,
            ADD_CHANNEL_MIN_POOL_CONTRIBUTION.sub(CHANNEL_DEACTIVATION_FEES)
          );
      });
      ...
      it("Should allow allow admin channel destruction after time is reached + 14days", async function () {
        ...
        const txn =
          EPNSCoreV1Proxy.connect(ADMINSIGNER).destroyTimeBoundChannel(
            CHANNEL_CREATOR
          );
        await expect(txn).to.be.revertedWith(
          "EPNSCoreV1_5::destroyTimeBoundChannel: Invalid Caller or Channel has not Expired Yet"
      ...
      it("Gives refunds on channel destroy", async function () {
        ...
        const expectedUserBalance = userBalBefore.add(
          ADD_CHANNEL_MIN_POOL_CONTRIBUTION.sub(CHANNEL_DEACTIVATION_FEES)
        );
      ...
      it("Reverts on destroying others channel", async function () {
        ...
        await passTime(ONE_DAY);
        const txn =
          EPNSCoreV1Proxy.connect(BOBSIGNER).destroyTimeBoundChannel(
            CHANNEL_CREATOR
          );
      ...
      it("Channel destruction unsubscribes to all", async function () {
        ...
        var isSubscribedToOwnChannel = await EPNSCommV1Proxy.isUserSubscribed(
          CHANNEL_CREATOR,
          CHANNEL_CREATOR
        );
        var isSubscribedTOChannelAlerter =
          await EPNSCommV1Proxy.isUserSubscribed(
            ethers.constants.AddressZero,
            CHANNEL_CREATOR
          );
        var isEPNSAdminSubscribed = await EPNSCommV1Proxy.isUserSubscribed(
          CHANNEL_CREATOR,
          ADMIN
        );
      ...
      it("Should delete channel info after channel destroy", async function () {
        ...
        const fullChannelInfo = await EPNSCoreV1Proxy.channels(CHANNEL_CREATOR);
        // destroy channel
        await passTime(ONE_DAY);
        await EPNSCoreV1Proxy.connect(
          CHANNEL_CREATORSIGNER
