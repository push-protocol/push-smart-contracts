...
import {CCRConfig} from "../utils/CCRConfig.sol";
contract BaseCCRTest is BasePushCommTest, CCRConfig {
    ...
    function setUpChain1(string memory url) internal {
        ...
        pushNttToken = Push(address(ArbSepolia.PUSH_NTT_SOURCE));
    ...
    function setUpChain2(string memory url) internal {
        switchChains(url);
        BasePushCommTest.setUp();
        pushNttToken = Push(address(EthSepolia.PUSH_NTT_DEST));
        changePrank(actor.admin);
