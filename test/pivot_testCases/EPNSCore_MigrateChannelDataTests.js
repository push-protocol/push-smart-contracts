  ...
  describe("Testing migrateChannelData FUnction", function()
      {
          ...
          const testChannel = ethers.utils.toUtf8Bytes("test-channel-hello-world");
            ...
            */
            ...
            it("Should revert if Admin is Not the Caller" , async ()=>{
               ...
               const tx = EPNSCoreV1Proxy.connect(BOBSIGNER).migrateChannelData(channelArray, channelTypeArray, amountArray);
            ...
            it("Should revert  if 'isMigrationComplete' flag is TRUE " , async ()=>{
               ...
               const tx = EPNSCoreV1Proxy.connect(ADMINSIGNER).migrateChannelData(channelArray, channelTypeArray, amountArray);

               expect(isMigrationComplete_before).to.equal(false);
               expect(isMigrationComplete_after).to.equal(true);
               await expect(tx).to.be.revertedWith('Migration of Channel Data is Complete Already');
            })
          ...
          it("Migration should transfer the Right Amount of DAI to EPNS PROXY from ADMIN's Balance", async function(){
            ...
            await EPNSCoreV1Proxy.connect(ADMINSIGNER).migrateChannelData(channelArray, channelTypeArray, amountArray);
          ...
          const tx = await EPNSCoreV1Proxy.connect(ADMINSIGNER).migrateChannelData(channelArray, channelTypeArray, amountArray);
        ...
        it("EPNS Core Should Interact with EPNS Communcator and make the necessary Subscriptions", async function(){
          ...
          const isBOBSubscribed_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(BOB, BOB);
          const isBOBSubscribedToEPNS_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(EPNS_ALERTER, BOB);
          const isAdminSubscribedToBOB_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(BOB, ADMIN);

          const isCHARLIESubscribed_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(CHARLIE, CHARLIE);
          const isCHARLIESubscribedToEPNS_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(EPNS_ALERTER, CHARLIE);
          const isAdminSubscribedToCHARLIE_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(CHARLIE, ADMIN);

          const isUSER1Subscribed_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(USER1, USER1);
          const isUSER1SubscribedToEPNS_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(EPNS_ALERTER, USER1);
          const isAdminSubscribedToUSER1_before = await EPNSCommunicatorV1Proxy.isUserSubscribed(USER1, ADMIN);
          ...
          const isBOBSubscribed_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(BOB, BOB);
          const isBOBSubscribedToEPNS_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(EPNS_ALERTER, BOB);
          const isAdminSubscribedToBOB_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(BOB, ADMIN);

          const isCHARLIESubscribed_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(CHARLIE, CHARLIE);
          const isCHARLIESubscribedToEPNS_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(EPNS_ALERTER, CHARLIE);
          const isAdminSubscribedToCHARLIE_after = await EPNSCommunicatorV1Proxy.isUserSubscribed(CHARLIE, ADMIN);
