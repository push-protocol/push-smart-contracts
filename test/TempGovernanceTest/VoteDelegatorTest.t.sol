...
contract PushVoteDelegatorTest is Test, BaseTest{
  ...
  function __deploy(address _deployer, address _delegatee) public returns (PushVoteDelegator) {
   vm.assume(_deployer != address(0));
    ...
    vm.prank(_deployer);
    PushVoteDelegator _surrogate = new PushVoteDelegator(IPUSH(address(govToken)), _delegatee);
    return _surrogate;
  }
}
...
contract Constructor is PushVoteDelegatorTest {
  function testFuzz_DelegatesToDeployer(address _deployer, address _delegatee) public {
    PushVoteDelegator _surrogate = __deploy(_deployer, _delegatee);
  ...
  function test_DelegationWorks_with_EOA() public {
    ...
    PushVoteDelegator _surrogate = __deploy(actor.admin, _delegatee);
    govToken.mint(address(_surrogate), 1000);

    uint256 balSurrogate = govToken.balanceOf(address(_surrogate));
  ...
  function test_DelegationWorks_with_CONTRACT() public {
    ...
    PushVoteDelegator _surrogate = new PushVoteDelegator(IPUSH(address(govToken)), actor.charlie_channel_owner);
    ...
    govToken.transfer(address(_surrogate), 1129);
    uint256 balSurrogate = govToken.balanceOf(address(_surrogate));
    ...
    address delegateState = govToken.delegates(address(_surrogate));
    ...
    govToken.transfer(address(_surrogate), 1000);
