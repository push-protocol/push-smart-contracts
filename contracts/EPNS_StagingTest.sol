...
contract EPNSStagingTest is Initializable, ReentrancyGuard  {
    ...
    *************** */
    ...
    event sendNotificationNew(address indexed channel, bytes indexed recipient, bytes identity);
  ...
  function sendNotificationAdvanced
  (
    address _channel,
    address _delegate,
  ...
  ) public
  {
    if(_recipient.length == 20){
      ...
      emit sendNotificationNew(_channel, _recipient, _identity);
  }
