...
contract MigrateEpochDetails_Test is BasePushFeePoolStaking{
    ...
    function setUp() public virtual override {
        ...
        _epochToTotalStakedWeights.push(uint256(19999));
        _epochToTotalStakedWeights.push(uint256(29999));
        _epochToTotalStakedWeights.push(uint256(39999));
    }
    ...
    function test_MigrateBeforeMigrationCompleted() public whenCallerIsAdmin() whenMigrationNotComplete(){
        ...
        uint expectedEpochReward_2ndIndex = _epochRewardss[1];
        uint actualEpochReward_2ndIndex = feePoolStaking.epochRewards(2);
        assertEq(actualEpochReward_2ndIndex, expectedEpochReward_2ndIndex);

        // Verifying epochToTotalStakedWeight
        uint expectEdepochToTotalStakedWeight_3rdIndex = _epochToTotalStakedWeights[2];
        uint actualEpochToTotalStakedWeight_3rdIndex = feePoolStaking.epochToTotalStakedWeight(3);
