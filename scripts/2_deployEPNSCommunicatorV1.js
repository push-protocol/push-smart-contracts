...
async function setupAllContracts() {
  ...
  const EPNSCommunicator = await deployContract("EPNSCommunicator", [], "EPNSCommunicator");
  deployedContracts.push(EPNSCommunicator)
  ...
  const EPNSCommunicatorProxy = await deployContract("EPNSCommunicatorProxy", [
    EPNSCommunicator.address,
    adminSigner.address,
    PUSH,
    WETH_ADDRESS,
    UNISWAP_ROUTER,
    ...
    ADAI,
    referralCode,
  ], "EPNSCommunicatorProxy");

  deployedContracts.push(EPNSCommunicatorProxy)
